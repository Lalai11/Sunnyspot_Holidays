...
<div class="content all-space-container">
  <?php
    ...
    $cabinID = $_GET['CABINID'];
        ...
        // check if the cabin is also on tblCabinInc. If yes, it can not be deleted because of the FK constraint.
        $query = "SELECT * FROM tblCabinInc WHERE cabinID = $cabinID";
        ...
        if($num_rows >0) {
  ?>
      <div class="alert alert-info" role="alert">Sorry, this cabin can not be deleted.
  <?php
        mysqli_close($link);
        exit();
  ?>
  ...
  <?php
        ...
        if(!$result) {
  ?>
      <div class="alert alert-info" role="alert">Error deleting cabin from database.
  <?php
        echo "Query error: " . mysqli_error($link);
  ?>
      </div>
  ...
  <?php
        echo("Cabin $cType deleted successfully.");
  ?>
      </div>
  <?php
        }
  ?>
  <a href="cabinDeleteTable.php"><span class="return-button">Return</span></a>
